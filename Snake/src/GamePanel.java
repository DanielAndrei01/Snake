...
public class GamePanel extends JPanel implements ActionListener{
    ...
    public void startGame() {
    	newFood();
        running = true;
        int initialDelay = DELAY - (foodEaten * 10);
    ...
    public void checkFood() {
    	if(x[0]==foodX&& y[0]==foodY) {
    		...
    		foodEaten++;
    		newFood();
    		 int newDelay = DELAY - foodEaten;
    	        timer.setDelay(newDelay);
    	}
    		
    }

    public void checkLovituri() {
    	//verificare corp loveste corp
    	for(int i=bodyParts; i>0; i--) {
    		if((x[0] ==x[i]&&y[0]==y[i])) {
    			running=false;
    		} 
    	}
    	//verificare loveste zid stanga
    	if(x[0]<0) running=false;
    	//verificare loveste zid dreapta
    	if(x[0]>SCREEN_WIDTH-1) running=false;
    	//verificare loveste top
    	if(y[0]<0) running =false;
    	//verificare loveste jos
